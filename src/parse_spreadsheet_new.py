...
class Spreadsheet2RDF(Parser):
    ...
    def __init__(self, file, format="ODT", sheet_index=1, conf_file=None, context_graph_file=None, corpus_uri_prefix="http://mycorpus", graph_identifier=None):
        ...
        # get column, inject column name from conf, split values.

        self.context_graph = Graph()
        if context_graph_file is not None:
            self.context_graph.parse(context_graph_file)

        resources = Spreadsheet2RDF._configuration_files_resources(self.conf['resources'], self.context_graph)
    ...
    @staticmethod
    def _configuration_files_resources(resources, context_graph):
        res = dict()
        for resource in resources:
            #resource_set = ResourceSet(resource["http://www.w3.org/1999/02/22-rdf-syntax-ns#type"])
            if 'lookup' in resource:
                pass
                #resource_set.lookup(resource['lookup'])
